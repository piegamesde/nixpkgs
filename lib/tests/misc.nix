...
in runTests {
  ...
  */
  ...
  testFindFirstEmpty = {
    expr =
      findFirst (abort "when the list is empty, the predicate is not needed")
      null [ ];
    expected = null;
  };
  ...
  testFindFirstNone = {
    expr = builtins.tryEval (findFirst (x: x == 2) null [
      1
