      ...
      final =
        {
          ...
          canExecute = platform: final.isAndroid == platform.isAndroid && parse.isCompatible final.parsed.cpu platform.parsed.cpu && final.parsed.kernel == platform.parsed.kernel;
        ...
        // (
          let
            selectEmulator =
              ...
              in
              if final.parsed.kernel.name == pkgs.stdenv.hostPlatform.parsed.kernel.name && pkgs.stdenv.hostPlatform.canExecute final then
                ''${pkgs.runtimeShell} -c '"$@"' --''
              else if final.isWindows then
