...
let
  ...
  inherit (lib.trivial) isFunction pipe inPureEvalMode;
in
{
  ...
  toSource =
    ...
    then
      throw ''
        ...
        }, which is not under the `root` (${toString root}). Potential solutions:
            - Set `root` to ${
              toString fileset._internalBase
  ...
  */
  gitTrackedWith =
    ...
    */
    ...
    if inPureEvalMode then
      throw
        "lib.fileset.gitTrackedWith: This function is currently not supported in pure evaluation mode, since it currently relies on `builtins.fetchGit`. See https://github.com/NixOS/nix/issues/9292."
    ...
    then
      ...
      throw
        "lib.fileset.gitTrackedWith: Expected the second argument to be a path, but it's a ${
          typeOf path
        } instead."
    ...
    # even though `git ls-files` wouldn't return any files in that case.
      ...
      throw
        "lib.fileset.gitTrackedWith: Expected the second argument (${
          toString path
        }) to point to a local working tree of a Git repository, but it's not."
    else
      _mirrorStorePath path fetchResult.outPath;
