...
let
  ...
  inherit (lib.path) append splitRoot;
    ...
    if value._type or "" == "fileset" then
      if value._internalVersion > _currentVersion then
        ...
        let
          ...
          migrationsToApply =
            sublist value._internalVersion (_currentVersion - value._internalVersion)
              migrations;
        in
    ...
    if firstWithBase != null && differentIndex != null then
      throw ''
        ${functionContext}: Filesystem roots are not the same:
            ...
            ${(elemAt list differentIndex).context}: Filesystem root is "${
              toString (elemAt filesets differentIndex)._internalBaseRoot
            }"
      ...
      fromFile =
        ...
        if
          predicate {
            ...
            "lib.fileset.fileFilter: The predicate function passed as the first argument must be able to handle extra attributes for future compatibility. If you're using `{ name, file }:`, use `{ name, file, ... }:` instead." =
              null;
          }
        then
          type
