    ...
    if !isEfi && !pkgs.stdenv.hostPlatform.isx86 then
      ...
      ''
        def assemble_qemu_flags():
            flags = "-cpu max"
            ${if (system == "x86_64-linux" || system == "i686-linux") then ''flags += " -m 1024"'' else ''flags += " -m 768 -enable-kvm -machine virt,gic-version=host"''}
            return flags
      ...
      nodes = {
        ...
        machine =
          { pkgs, ... }:
          {
            ...
            virtualisation.rootDevice = if grubVersion == 1 then "/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_drive2" else "/dev/vdb";
