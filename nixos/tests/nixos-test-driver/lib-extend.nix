...
let
  patchedPkgs = pkgs.extend (
    new: old: {
      lib = old.lib.extend (self: super: { sorry_dave = "sorry dave"; });
    }
  );
  ...
  testBody = {
    ...
    nodes = {
      machine =
        { lib, ... }: { environment.etc."got-lib-overlay".text = lib.sorry_dave; };
    };
...
in
{
  ...
  runNixOSTest =
    evaluationRunNixOSTest.driver.nodes.machine.system.build.toplevel;
}
