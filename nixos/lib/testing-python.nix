...
, extraConfigurations ? [ ] }:
let nixos-lib = import ./default.nix { inherit (pkgs) lib; };

in pkgs.lib.throwIf (args ? specialArgs) ''
...
'' rec {
  ...
  makeTest = { machine ? null, nodes ? { }, testScript, enableOCR ? false
    , name ? "unnamed", skipTypeCheck ? false
      # Skip linting (mainly intended for faster dev cycles)
    , skipLint ? false, passthru ? { }, meta ? { }, # For meta.position
    ...
    , extraPythonPackages ? (_: [ ]), interactive ? { } }@t:
    let
      testConfig = (evalTest {
        imports = [
          {
