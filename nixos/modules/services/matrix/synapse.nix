...
let
  ...
  registerNewMatrixUser =
    let
      ...
      listener =
        lib.findFirst (listener: lib.any (resource: lib.any (name: name == "client") resource.names) listener.resources) (lib.last cfg.settings.listeners)
    ...
    pkgs.writeShellScriptBin "matrix-synapse-register_new_matrix_user" ''
      exec ${cfg.package}/bin/register_new_matrix_user \
        ...
        "${listenerProtocol}://${if (isIpv6 bindAddress) then "[${bindAddress}]" else "${bindAddress}"}:${builtins.toString listener.port}/"
    '';
in
{
          ...
          submodule {
            ...
            options = {
              ...
              media_store_path = mkOption {
                type = types.path;
                default = if lib.versionAtLeast config.system.stateVersion "22.05" then "${cfg.dataDir}/media_store" else "${cfg.dataDir}/media";
