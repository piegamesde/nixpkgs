          ...
          config.assertions = [
            ...
            }
            {
              ...
              message = "Either ${showPath [ "acl" ]} or ${
                  showPath [ "aclFile" ]
                } is mandatory if ${showPath [ "settings" "useacl" ]} is true";
            }
            {
              assertion = config.usersFile != null -> config.mergedConfig.useacl
                != false;
              message = "${showPath [ "settings" "useacl" ]} is required when ${
                  showPath [ "usersFile" ]
  ...
  options = {
    services.dokuwiki = {
      ...
      sites = mkOption {
        ...
        default = { };
        description =
          lib.mdDoc "Specification of one or more DokuWiki sites to serve";
      };
