...
let
  ...
  provisioningScript = pkgs.writeShellScript "post-start-provision" ''
    ...
    provision_result=$(${
      getExe pkgs.influxdb2-provision
    } ${provisionState} "$INFLUX_HOST" "$(< "$CREDENTIALS_DIRECTORY/admin-token")")
            ...
            authName: auth: [
              ...
              (
                ...
                in
                {
                  ...
                  message = "influxdb2: provision.organizations.${orgName}.auths.${authName}: Refers to invalid buckets in readBuckets: ${
                      toString unknownBuckets
                    }";
                }
              )
              (
                ...
                {
                  assertion = unknownBuckets == [ ];
                  message = "influxdb2: provision.organizations.${orgName}.auths.${authName}: Refers to invalid buckets in writeBuckets: ${
                      toString unknownBuckets
                    }";
                }
              )
