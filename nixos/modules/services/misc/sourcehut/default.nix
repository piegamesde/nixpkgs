...
let
  ...
  configIniOfService = srv:
    ...
    (filterAttrs (k: v: v != null) (mapAttrs (section: v:
      let srvMatch = builtins.match "^([a-z]*)\\.sr\\.ht(::.*)?$" section;
  ...
  commonServiceSettings = srv: {
    origin = mkOption {
      description =
        lib.mdDoc "URL ${srv}.sr.ht is being served at (protocol://domain)";
      type = types.str;
    ...
    oauth-client-secret = mkOption {
      description =
        lib.mdDoc "${srv}.sr.ht's OAuth client secret for meta.sr.ht.";
      type = types.path;
      apply = s: "<" + toString s;
    };
  };
    ...
    settings = mkOption {
      type = lib.types.submodule {
        ...
        options."sr.ht" = {
          ...
          site-name = mkOption {
            description =
              lib.mdDoc "The name of your network of sr.ht-based sites.";
            type = types.str;
            default = "sourcehut";
          };
          source-url = mkOption {
        ...
        options.mail = {
          ...
          error-to =
            mkOptionNullOrStr "Address receiving application exceptions";
          error-from =
            mkOptionNullOrStr "Address sending application exceptions";
        ...
        options.objects = {
          s3-upstream = mkOption {
            description =
              lib.mdDoc "Configure the S3-compatible object storage service.";
            type = with types; nullOr str;
            default = null;
          };
        ...
        options."builds.sr.ht" = commonServiceSettings "builds" // {
          allow-free =
            mkEnableOption (lib.mdDoc "nonpaying users to submit builds");
          redis = mkOption {
            description =
          ...
          shell = mkOption {
            ...
            '';
            type =
              types.enum [ "/usr/bin/master-shell" "/usr/bin/runner-shell" ];
        ...
        options."lists.sr.ht" = commonServiceSettings "lists" // {
          allow-new-lists =
            mkEnableOption (lib.mdDoc "Allow creation of new lists");
          notify-from = mkOption {
            description =
              lib.mdDoc "Outgoing email for notifications generated by users.";
            type = types.str;
        ...
        options."meta.sr.ht::billing" = {
          ...
          stripe-public-key = mkOptionNullOrStr
        ...
        options."meta.sr.ht::settings" = {
          ...
          onboarding-redirect = mkOption {
            description =
              lib.mdDoc "Where to redirect new users upon registration.";
            type = types.str;
        ...
        options."pages.sr.ht" = commonServiceSettings "pages" // {
          ...
          max-site-size = mkOption {
            description =
              lib.mdDoc "Maximum size of any given site (post-gunzip), in MiB.";
            type = types.int;
            default = 1024;
          };
  ...
  imports = [
      ...
      mainService = mkMerge [
        baseService
        {
          serviceConfig.StateDirectory =
            [ "sourcehut/gitsrht" "sourcehut/gitsrht/repos" ];
    ...
    in {
      ...
      mainService = mkMerge [
        ...
        {
          serviceConfig.StateDirectory =
            [ "sourcehut/hgsrht" "sourcehut/hgsrht/repos" ];
        }
      ];
      port = 5010;
      webhooks = true;
      ...
      extraConfig = {
        services.nginx = mkIf cfg.nginx.enable {
          virtualHosts."hub.${domain}" =
            mkMerge [ { serverAliases = [ domain ]; } cfg.nginx.virtualHost ];
        };
      };
    })

    (import ./service.nix "lists" (let srvsrht = "listssrht";
    in {
      inherit configIniOfService;
      port = 5006;
      webhooks = true;
    ...
    (import ./service.nix "meta" {
      ...
      extraServices.metasrht-api = {
        ...
        '' + concatStringsSep "\n\n" (attrValues (mapAttrs (k: s:
          ...
          in optionalString (srvMatch != null && cfg.${srv}.enable
      ...
      extraConfig = mkMerge [
        {
          assertions = [{
            assertion = let s = cfg.settings."meta.sr.ht::billing";
            in s.enabled == "yes"
