            ...
            imap0 (i: f: f i) (
              mapAttrsToList
                (bss: bssCfg: bssIdx: ''
                  ...
                  ${concatMapStrings
                    (script: ''
                      ${script} "$hostapd_config_file" "$mac_allow_file" "$mac_deny_file"
                    '')
                              ...
                              in
                              {
                                ...
                                "20-addMacAllowFile" = mkIf (bssCfg.macAllowFile != null) (
                                  pkgs.writeShellScript "add-mac-allow-file" ''
                                    ...
                                    grep -Eo '^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})' ${
                                      escapeShellArg bssCfg.macAllowFile
                                    } >> "$MAC_ALLOW_FILE"
                                  ''
                                );
                                ...
                                "20-addMacDenyFile" = mkIf (bssCfg.macDenyFile != null) (
                                  pkgs.writeShellScript "add-mac-deny-file" ''
                                    MAC_DENY_FILE=$3
                                    grep -Eo '^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})' ${
                                      escapeShellArg bssCfg.macDenyFile
                                    } >> "$MAC_DENY_FILE"
                                  ''
                                );
  ...
  imports =
    ...
    [
      ...
      (mkRemovedOptionModule
        ...
        (
          renamedOptionMessage
            "It has been replaced by `services.hostapd.radios.«interface».networks.«network».logLevel`."
        )
      )
      (mkRemovedOptionModule
        [
        ...
        (
          renamedOptionMessage
            "It has been replaced by `services.hostapd.radios.«interface».networks.«network».group`."
        )
      )
      (mkRemovedOptionModule
        [
      ...
      (mkRemovedOptionModule
        ...
        (
          renamedOptionMessage
            "It has been replaced by `services.hostapd.radios.«interface».networks.«network».authentication.mode`."
        )
      )
      (mkRemovedOptionModule
        [
