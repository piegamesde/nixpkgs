...
let
  ...
  toIniJson = with generators; toINI { mkKeyValue = mkKeyValueDefault { mkValueString = builtins.toJSON; } "="; };
  ...
  toConfigFile =
    ...
    if cfg'.configFile != null then
      ...
      pkgs.writeText "vdirsyncer-${name}.conf" (
        toIniJson (
          {
            general = cfg'.config.general // (lib.optionalAttrs (cfg'.config.statusPath == null) { status_path = "/var/lib/vdirsyncer/${name}"; });
          }
