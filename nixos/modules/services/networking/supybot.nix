...
in
{
  options = {
    ...
    services.supybot = {
      ...
      stateDir = mkOption {
        type = types.path;
        default = if versionAtLeast config.system.stateVersion "20.09" then "/var/lib/supybot" else "/home/supybot";
  ...
  config = mkIf cfg.enable {
    ...
    ] ++ (flip mapAttrsToList cfg.plugins (name: dest: "L+ '${cfg.stateDir}/plugins/${name}' - - - - ${dest}"));
  };
}
