...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.gateone = with pkgs; {
      ...
      serviceConfig = {
        ExecStart = "${pythonPackages.gateone}/bin/gateone --settings_dir=${cfg.settingsDir} --pid_file=${cfg.pidDir}/gateone.pid --gid=${toString config.ids.gids.gateone} --uid=${
            toString config.ids.uids.gateone
          }";
        User = "gateone";
        Group = "gateone";
