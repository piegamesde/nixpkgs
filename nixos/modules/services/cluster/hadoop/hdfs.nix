        ...
        mkMerge [
          {
            ...
            networking.firewall.allowedTCPPorts = mkIf ((builtins.hasAttr "openFirewall" serviceOptions) && serviceOptions.openFirewall) allowedTCPPorts;
          }
          extraConfig
        ]
      )
...
in
{
  ...
  config = mkMerge [
    (hadoopServiceConfig {
      ...
      preStart = (mkIf cfg.hdfs.namenode.formatOnInit "${cfg.package}/bin/hdfs --config ${hadoopConf} namenode -format -nonInteractive || true");
    })
