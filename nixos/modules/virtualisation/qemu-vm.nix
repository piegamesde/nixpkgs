...
let
  ...
  bootConfiguration = if cfg.useDefaultFilesystems then
    ...
    else if cfg.directBoot.enable then
...
in {
  ...
  options = {
    ...
    virtualisation.bootLoaderDevice = mkOption {
      ...
      defaultText =
        literalExpression ''lookupDriveDeviceName "root" cfg.qemu.drives'';
      example = "/dev/vda";
      description = lib.mdDoc ''
    ...
    virtualisation.qemu = {
      package = mkOption {
        ...
        defaultText =
          literalExpression "config.virtualisation.host.pkgs.qemu_kvm";
    ...
    virtualisation.mountHostNixStore = mkOption {
      ...
      defaultText =
        literalExpression "!cfg.useNixStoreImage && !cfg.useBootLoader";
      description = lib.mdDoc ''
  ...
  config = {
    ...
    assertions = lib.concatLists (lib.flip lib.imap cfg.forwardPorts (i: rule: [
      ...
      {
        assertion = cfg.directBoot.initrd
          != options.virtualisation.directBoot.initrd.default
    ...
    boot.initrd.kernelModules =
      optionals (cfg.useNixStoreImage && !cfg.writableStore) [ "erofs" ];
