...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc = {
      "greetd/regreet.css" = if lib.isPath cfg.extraCss then { source = cfg.extraCss; } else { text = cfg.extraCss; };

      "greetd/regreet.toml".source = if lib.isPath cfg.settings then cfg.settings else settingsFormat.generate "regreet.toml" cfg.settings;
    };
