...
in
{
  ...
  config = mkIf cfg.enable {
    boot = {
      ...
      extraModprobeConfig = mkIf (config.hardware.bluetooth.enable && (lib.versionOlder config.boot.kernelPackages.kernel.version "5.12")) "options bluetooth disable_ertm=1";
