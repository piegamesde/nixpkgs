...
let
  ...
  kernelParam =
    ...
    name: value:
    assert lib.asserts.assertMsg (!lib.strings.hasInfix "=" name)
...
in
{
  options.hardware.cpu.x86.msr =
    ...
    with lib.types; {
      enable = mkEnableOption (
        mdDoc
          "the `msr` (Model-Specific Registers) kernel module and configure `udev` rules for its devices (usually `/dev/cpu/*/msr`)"
      );
      owner = mkOption {
        type = str;
        default = "root";
    ...
    assertions = [
      {
        ...
        message = ''
          Owner '${cfg.owner}' set in `${opt.owner}` is not configured via `${options.users.users}."${cfg.owner}"`.'';
      }
      {
        assertion = isDefaultGroup || (hasAttr cfg.group config.users.groups);
        message = ''
          Group '${cfg.group}' set in `${opt.group}` is not configured via `${options.users.groups}."${cfg.group}"`.'';
      }
    ];
  ...
  meta = with lib; { maintainers = with maintainers; [ lorenzleutgeb ]; };
}
