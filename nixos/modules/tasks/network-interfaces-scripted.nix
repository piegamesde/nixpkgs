    ...
    systemd.services = let
      ...
      createVswitchDevice = n: v:
        nameValuePair "${n}-netdev" (let
          deps = concatLists (map deviceDependency (attrNames
            (filterAttrs (_: config: config.type != "internal") v.interfaces)));
          internalConfigs = map (i: "network-addresses-${i}.service") (attrNames
            (filterAttrs (_: config: config.type == "internal") v.interfaces));
            ...
            ovs-vsctl ${
              ...
              ${
                concatStrings (mapAttrsToList (name: config:
                  ...
                  " -- set interface ${name} type=${config.type}") v.interfaces)
              } \
              ${
