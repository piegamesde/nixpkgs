...
in {
  ...
  config = {
    ...
    assertions = let
      ...
      resizableFSes = [ "ext3" "ext4" "btrfs" "xfs" ];
      notAutoResizable = fs:
        fs.autoResize && !(builtins.elem fs.fsType resizableFSes);
    in [
      {
      ...
      }
      {
        ...
        message =
          let fs = head (filter (fs: fs.formatOptions != null) fileSystems);
          in ''
