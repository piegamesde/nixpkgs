...
let
  ...
  stage1Crypttab = pkgs.writeText "initrd-crypttab" (lib.concatStringsSep "\n"
    (lib.mapAttrsToList (n: v:
      ...
      in "${n} ${v.device} ${if v.keyFile == null then "-" else v.keyFile} ${
        ...
        attrsOf (submodule ({ name, ... }: {
          options = {
            ...
            device = mkOption {
              ...
              type = types.str;
              description =
                lib.mdDoc "Path of the underlying encrypted block device.";
            };
                ...
                nullOr (submodule {
                  options = {
                    ...
                    encryptedPass = mkOption {
                      type = types.path;
                      description =
                        lib.mdDoc "Path to the GPG encrypted passphrase.";
                    };
            ...
            fido2 = {
              ...
              gracePeriod = mkOption {
                ...
                type = types.int;
                description =
                  lib.mdDoc "Time in seconds to wait for the FIDO2 key.";
              };
                ...
                nullOr (submodule {
                  options = {
                    ...
                    slot = mkOption {
                      ...
                      type = types.int;
                      description =
                        lib.mdDoc "Which slot on the YubiKey to challenge.";
                    };
                    ...
                    */
                    storage = {
                      ...
                      fsType = mkOption {
                        ...
                        type = types.str;
                        description =
                          lib.mdDoc "The filesystem of the unencrypted device.";
                      };
  ...
  config = mkIf (luks.devices != { } || luks.forceLuksSupportInInitrd) {
    ...
    boot.initrd.extraUtilsCommands = let
      pbkdf2-sha512 =
        ...
        '';
    in mkIf (!config.boot.initrd.systemd.enable) ''
