          ...
          ++ (
            let
              ...
              operationalStateRanges = concatLists (
                imap0 (i: min: map (max: "${min}:${max}") (drop i operationalStates))
                  operationalStates
              );
            in
    ...
    mkMerge [
      ...
      # .link units are honored by udev, no matter if systemd-networkd is enabled or not.
      {
        systemd.network.units =
          mapAttrs' (n: v: nameValuePair "${n}.link" (mkUnit linkToUnit v))
            cfg.links;
      ...
      (mkIf config.systemd.network.enable {
        ...
        systemd.network.units =
          mapAttrs' (n: v: nameValuePair "${n}.netdev" (mkUnit netdevToUnit v))
            cfg.netdevs
          // mapAttrs' (n: v: nameValuePair "${n}.network" (mkUnit networkToUnit v))
            cfg.networks;
        ...
        systemd.services.systemd-networkd =
          let
            isReloadableUnitFileName =
              unitFileName: strings.hasSuffix ".network" unitFileName;
            reloadableUnitFiles =
              attrsets.filterAttrs (k: v: isReloadableUnitFileName k)
                unitFiles;
            nonReloadableUnitFiles =
              attrsets.filterAttrs (k: v: !isReloadableUnitFileName k)
                unitFiles;
  ...
  config = mkMerge [
    ...
    (mkIf config.boot.initrd.systemd.enable {
      assertions = [
        {
          assertion =
            !config.boot.initrd.network.udhcpc.enable
            && config.boot.initrd.network.udhcpc.extraArgs == [ ];
          message = ''
