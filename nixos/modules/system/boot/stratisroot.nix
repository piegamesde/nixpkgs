    ...
    boot.initrd = {
      systemd = {
        ...
        extraBin = {
          ...
          thin_metadata_size =
            "${pkgs."thin-provisioning-tools"}/bin/thin_metadata_size";
        ...
        services = lib.attrsets.mapAttrs' (mountPoint: fileSystem: {
          ...
          value = {
            ...
            before =
              [ "initrd.target" "shutdown.target" "initrd-switch-root.target" ];
      ...
      availableKernelModules = [ "dm-thin-pool" "dm-crypt" ] ++ [
        "aes"
        "aes_generic"
        "blowfish"
        "twofish"
      ...
      services.udev.packages = [ pkgs.stratisd.initrd pkgs.lvm2 ];
    };
  };
}
