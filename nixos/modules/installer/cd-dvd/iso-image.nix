...
let
  ...
  buildMenuAdditionalParamsGrub2 = additional:
    let
      finalCfg = {
        name =
          "${config.isoImage.prependToMenuLabel}${config.system.nixos.distroName} ${config.system.nixos.label}${config.isoImage.appendToMenuLabel}";
...
in {
  options = {
    ...
    isoImage.makeBiosBootable = mkOption {
      ...
      default = pkgs.stdenv.buildPlatform.isx86
  ...
  config = {
    assertions = [
      {
        ...
        assertion = config.isoImage.makeBiosBootable
          -> pkgs.stdenv.hostPlatform.isx86;
    ...
    system.build.isoImage = pkgs.callPackage ../../../lib/make-iso9660-image.nix
      ({
        ...
        syslinux =
          if config.isoImage.makeBiosBootable then pkgs.syslinux else null;
      } // optionalAttrs
