...
in {
  ...
  analysis-icu = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash = if version == "7.17.10" then
        "sha256-D08CVW/qHpZZaKnploM4aCJ4bunvPjVmieDYr1d6jQA="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
      homepage =
        "https://github.com/elastic/elasticsearch/tree/master/plugins/analysis-icu";
      description =
  ...
  analysis-kuromoji = esPlugin rec {
    ...
    src = fetchurl {
      url =
        "https://artifacts.elastic.co/downloads/elasticsearch-plugins/${pluginName}/${pluginName}-${version}.zip";
      hash = if version == "7.17.10" then
        "sha256-cpgr2zPCpsLrmshWJWoGNcGl0X+bO/K4A9bMqLv8+H8="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  analysis-phonetic = esPlugin rec {
    ...
    src = fetchurl {
      url =
        "https://artifacts.elastic.co/downloads/elasticsearch-plugins/${pluginName}/${pluginName}-${version}.zip";
      hash = if version == "7.17.10" then
        "sha256-UmykO+hZDvlFhEbf7zL2bdw4j6NhByRBu9eH3F6/EtM="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
      homepage =
        "https://github.com/elastic/elasticsearch/tree/master/plugins/analysis-phonetic";
      description =
  ...
  discovery-ec2 = esPlugin rec {
    ...
    src = fetchurl {
      url =
        "https://artifacts.elastic.co/downloads/elasticsearch-plugins/${pluginName}/${pluginName}-${version}.zip";
      hash = if version == "7.17.10" then
        "sha256-Y/AbLfHSdocX0NQbnKm63gTWgwzssb4kpSwRqLozD9w="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  ingest-attachment = esPlugin rec {
    ...
    src = fetchurl {
      url =
        "https://artifacts.elastic.co/downloads/elasticsearch-plugins/${pluginName}/${pluginName}-${version}.zip";
      hash = if version == "7.17.10" then
        "sha256-QIYD7cGpJQg+csv/tekN6GFtdnuhYU6VyAXk7nY/uWs="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
      ...
      description =
        "Ingest processor that uses Apache Tika to extract contents";
      license = licenses.asl20;
    };
  };
  ...
  repository-s3 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash = if version == "7.17.10" then
        "sha256-L8lS+EPYuhNNTnP3ImeZsBQ5a5DAncs3qBFDWGWISRI="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-gcs = esPlugin rec {
    ...
    src = fetchurl {
      url =
        "https://artifacts.elastic.co/downloads/elasticsearch-plugins/${pluginName}/${pluginName}-${esVersion}.zip";
      hash = if version == "7.17.10" then
        "sha256-eXstbxlyS8WzW8u5YiMFXGpILCcEWrIb/IxXVzAGFLU="
      else
        throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
