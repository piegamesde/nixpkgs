...
, fetchFromGitLab, buildPackages, automake, autoconf, libiconv, libtool
, intltool, freetype, tradcpp, fontconfig, meson, ninja, ed, fontforge, libGL
, spice-protocol, zlib, libGLU, dbus, libunwind, libdrm, netbsd, ncompress, mesa
, udev, bootstrap_cmds, bison, flex, clangStdenv, autoreconfHook, mcpp, libepoxy
, openssl, pkg-config, llvm, libxslt, libxcrypt, ApplicationServices, Carbon
, Cocoa, Xplugin, xorg }:
...
in self: super:
{
  ...
  libXft = super.libXft.overrideAttrs (attrs: {
    outputs = [ "out" "dev" ];
    propagatedBuildInputs = attrs.propagatedBuildInputs or [ ]
      ++ [ xorg.libXrender freetype fontconfig ];
    configureFlags = attrs.configureFlags or [ ] ++ malloc0ReturnsNullCrossFlag;
  ...
  libpciaccess = super.libpciaccess.overrideAttrs (attrs: {
    patches = attrs.patches or [ ] ++ [
      (fetchpatch {
        url =
          "https://gitlab.freedesktop.org/xorg/lib/libpciaccess/-/commit/833c86ce15cee2a84a37ae71015f236fd32615d9.patch";
        hash = "sha256-6koQV+Vse7/OWwuWYrWmBUebHBT+5F32Kkn9V9j+m+Q=";
      })
    ];

    buildInputs = lib.optionals stdenv.hostPlatform.isNetBSD
      (with netbsd; [ libarch libpci ]);
  ...
  xf86inputkeyboard = brokenOnDarwin
    super.xf86inputkeyboard; # never worked: https://hydra.nixos.org/job/nixpkgs/trunk/xorg.xf86inputkeyboard.x86_64-darwin
  ...
  xf86inputvoid = brokenOnDarwin
    super.xf86inputvoid; # never worked: https://hydra.nixos.org/job/nixpkgs/trunk/xorg.xf86inputvoid.x86_64-darwin
  xf86videodummy = brokenOnDarwin
    super.xf86videodummy; # never worked: https://hydra.nixos.org/job/nixpkgs/trunk/xorg.xf86videodummy.x86_64-darwin
    ...
    let
      patchIn = name: layout:
        ...
        with lib; ''
          ...
          EOF
        '';
    in xorg.xkeyboardconfig.overrideAttrs (old: {
    ...
    super.xorgserver.overrideAttrs (attrs_passed:
      ...
      in if (!isDarwin) then {
        ...
        nativeBuildInputs = attrs.nativeBuildInputs
          ++ [ autoreconfHook bootstrap_cmds xorg.utilmacros xorg.fontutil ];
  ...
  xinit = (super.xinit.override {
    ...
    depsBuildBuild = [ buildPackages.stdenv.cc ];
    configureFlags = [ "--with-xserver=${xorg.xorgserver.out}/bin/X" ]
  ...
  xf86videointel = super.xf86videointel.overrideAttrs (attrs: {
    ...
    buildInputs = attrs.buildInputs
      ++ [ xorg.libXScrnSaver xorg.libXv xorg.pixman xorg.utilmacros ];
    nativeBuildInputs = attrs.nativeBuildInputs ++ [ autoreconfHook ];
