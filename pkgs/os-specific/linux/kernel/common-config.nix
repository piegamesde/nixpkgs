...
let
  ...
  options = {
    ...
    networking = {
      ...
      BPF_JIT_ALWAYS_ON = whenPlatformHasEBPFJit
        no; # whenPlatformHasEBPFJit yes; # see https://github.com/NixOS/nixpkgs/issues/79304
    ...
    sound = {
      ...
      SND_HDA_CODEC_CA0132_DSP = whenOlder "5.7"
        yes; # Enable DSP firmware loading on Creative Soundblaster Z/Zx/ZxR/Recon
      SND_OSSEMUL = yes;
    ...
    security = {
      ...
      RANDOM_TRUST_BOOTLOADER = whenOlder "6.2"
        (whenAtLeast "5.4" yes); # allow the bootloader to seed the RNG
