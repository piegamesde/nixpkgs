...
rec {
  ...
  recompileLibc = { tcc, pname, version, src, libtccOptions }:
    let
      ...
      libgetopt =
        library "libgetopt" mes-libc.CFLAGS "${mes-libc}/lib/libgetopt.c";
    in kaem.runCommand "${pname}-libs-${version}" { } ''
      mkdir -p ''${out}/lib
  ...
  buildTinyccMes =
    { pname, version, src, prev, buildOptions, libtccBuildOptions, meta }:
    let
        ...
        passthru.tests = rec {
          ...
          chain = result:
            kaem.runCommand "${pname}-chain-${version}" { } ''
              echo ${
                prev.compiler.tests.chain or prev.compiler.tests.get-version
              };
              ${result}/bin/tcc -version
              mkdir ''${out}
            '';
