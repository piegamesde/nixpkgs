{ stdenv, fetchurl, fetchFromGitHub, buildPackages, lib, self, version, sha256
, pkgsBuildBuild, pkgsBuildHost, pkgsBuildTarget, pkgsHostHost, pkgsTargetTarget
, zlib, config, passthruFun
, perlAttr ? "perl${lib.versions.major version}${lib.versions.minor version}"
, enableThreading ? true, coreutils, makeWrapper, enableCrypt ? true
, libxcrypt ? null, overrides ? config.perlPackageOverrides or (p:
...
in stdenv.mkDerivation (rec {
  ...
  passthru = let
    ...
    override = attr:
      let perl = attr.override (inputs' // { self = perl; });
      in perl;
  in passthruFun rec {
