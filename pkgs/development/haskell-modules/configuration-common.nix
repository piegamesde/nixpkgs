...
self: super:
{
  ...
  stylish-haskell =
    if lib.versionAtLeast super.ghc.version "9.2" then
      enableCabalFlag "ghc-lib" (if lib.versionAtLeast super.ghc.version "9.4" then super.stylish-haskell_0_14_4_0 else super.stylish-haskell)
    else
  ...
  clay = throwIfNot (super.clay.version == "0.14.0") "Remove clay jailbreak in configuration-common.nix when you see this eval error." (
  ...
  cabal-install-parsers = doJailbreak (dontCheck (super.cabal-install-parsers.override { Cabal-syntax = self.Cabal-syntax_3_10_1_0; }));
  ...
  hinit = doJailbreak (self.generateOptparseApplicativeCompletions [ "hi" ] (super.hinit.override { haskeline = self.haskeline_0_8_2_1; }));
    ...
    overrideCabal
      {
        passthru = {
          ...
          compilerConfig = pkgs.runCommand "hackage2nix-${self.ghc.haskellCompilerName}-config.yaml" { nativeBuildInputs = [ self.ghc ]; } ''
            cat > "$out" << EOF
