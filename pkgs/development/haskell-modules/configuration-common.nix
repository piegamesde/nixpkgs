...
self: super:
{
  ...
  hinit = doJailbreak (
    self.generateOptparseApplicativeCompletions [ "hi" ] (super.hinit.override { haskeline = self.haskeline_0_8_2_1; })
  );
    ...
    overrideCabal
      {
        passthru = {
          ...
          compilerConfig =
            pkgs.runCommand "hackage2nix-${self.ghc.haskellCompilerName}-config.yaml" { nativeBuildInputs = [ self.ghc ]; }
              ''
                cat > "$out" << EOF
