...
let
  ...
  packages = selectPackages haskellPackages ++ lib.optional withHoogle (hoogleWithPackages selectPackages);
  ...
  libDir =
    if isHaLVM then
      ...
      "$out/lib/${ghc.targetPrefix}${ghc.haskellCompilerName}" + lib.optionalString (ghc ? hadrian) "/lib";
