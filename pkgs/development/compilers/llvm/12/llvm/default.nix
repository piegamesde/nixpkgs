{
  ...
  buildLlvmTools,
  debugVersion ? false,
  doCheck ? stdenv.isLinux && (!stdenv.isx86_32) && (!stdenv.hostPlatform.isMusl) && (stdenv.hostPlatform == stdenv.buildPlatform),
...
stdenv.mkDerivation (
  rec {
    ...
    propagatedBuildInputs = optionals (stdenv.buildPlatform == stdenv.hostPlatform) [ ncurses ] ++ [ zlib ];
