...
(lib.filterAttrs (attr: _: (prev ? "${attr}")) {
  ...
  libcufile = prev.libcufile.overrideAttrs (
    oldAttrs: {
      ...
      autoPatchelfIgnoreMissingDeps =
        ...
        ++ lib.optionals (cudaVersionOlder "12.0") [ "libcufile.so.0" ];
    }
  );
  ...
  libcusolver = final.addBuildInputs prev.libcusolver (
    ...
    ++ lib.optionals (cudaVersionAtLeast "12.0") [ final.libnvjitlink.lib ]
    # Dependency from 12.1 and on
    ++ lib.optionals (cudaVersionAtLeast "12.1") [ final.libcusparse.lib ]
  );
  ...
  cuda_gdb = final.addBuildInputs prev.cuda_gdb (
    ...
    lib.optionals (cudaVersionAtLeast "12.0") [ pkgs.gmp ]
  );
  ...
  cuda_nvcc = prev.cuda_nvcc.overrideAttrs (
    _: {
      ...
      depsTargetTargetPropagated = [ final.setupCudaHook ];
    }
  );
