...
rustPlatform.buildRustPackage.override { cargo-auditable = cargo-auditable.bootstrap; } (
  {
    ...
    doInstallCheck = !stdenv.hostPlatform.isStatic && stdenv.hostPlatform.parsed.kernel.execFormat == lib.systems.parse.execFormats.elf;
    installCheckPhase = ''
  ...
  // lib.optionalAttrs (rust.toRustTarget stdenv.buildPlatform != rust.toRustTarget stdenv.hostPlatform) { HOST_PKG_CONFIG_PATH = "${pkgsBuildBuild.pkg-config}/bin/pkg-config"; }
)
