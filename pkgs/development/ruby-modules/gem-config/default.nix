...

{
  ...
  grpc = attrs: {
    nativeBuildInputs = [
      pkg-config
    ] ++ lib.optional stdenv.isDarwin cctools ++ lib.optional (lib.versionAtLeast attrs.version "1.53.0" && stdenv.isDarwin && stdenv.isAarch64) autoSignDarwinBinariesHook;
    buildInputs = [ openssl ];
  ...
  tzinfo =
    ...
    lib.optionalAttrs (lib.versionAtLeast attrs.version "1.0") {
      ...
      postPatch =
        let
          path = if lib.versionAtLeast attrs.version "2.0" then "lib/tzinfo/data_sources/zoneinfo_data_source.rb" else "lib/tzinfo/zoneinfo_data_source.rb";
        in
        ''
