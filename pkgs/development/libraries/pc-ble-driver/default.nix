...
stdenv.mkDerivation rec {
  ...
  cmakeFlags = [ "-DNRF_BLE_DRIVER_VERSION=${version}" ] ++ lib.optionals (stdenv.isDarwin && stdenv.isAarch64) [ "-DARCH=arm64" ];
  ...
  propagatedBuildInputs = [ ] ++ lib.optionals stdenv.isDarwin [ IOKit ] ++ lib.optionals stdenv.isLinux [ udev ];
