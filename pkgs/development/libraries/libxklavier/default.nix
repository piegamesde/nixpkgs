{ lib, stdenv, fetchgit, fetchpatch, autoreconfHook, pkg-config, gtk-doc
, xkeyboard_config, libxml2, xorg, docbook_xsl, glib, isocodes
, gobject-introspection, withDoc ? (stdenv.buildPlatform == stdenv.hostPlatform)
}:
...
stdenv.mkDerivation rec {
  ...
  patches = [ ./honor-XKB_CONFIG_ROOT.patch ] ++ lib.optionals stdenv.isDarwin [
    (fetchpatch {
      url =
