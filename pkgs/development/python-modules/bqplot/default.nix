{ lib, buildPythonPackage, pythonOlder, fetchPypi, jupyter-packaging, bqscales
, ipywidgets, numpy, pandas, traitlets, traittypes }:
...
buildPythonPackage rec {
  ...
  propagatedBuildInputs =
    [ bqscales ipywidgets numpy pandas traitlets traittypes ];

  # no tests in PyPI dist, and not obvious to me how to build the js files from GitHub
  doCheck = false;

  pythonImportsCheck = [ "bqplot" "bqplot.pyplot" ];
