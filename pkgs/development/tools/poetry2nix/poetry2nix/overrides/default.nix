...
let
  addBuildSystem =
      ...
      buildSystem =
        if builtins.isAttrs attr then
          let
            ...
            intendedBuildSystem = if attr.buildSystem == "cython" then self.python.pythonForBuild.pkgs.cython else self.${attr.buildSystem};
          in
    ...
    (
      ...
      if (attr == "flit-core" || attr == "flit" || attr == "hatchling") && !self.isPy3k then
        drv
      else if drv == null then
        null
      else if drv ? overridePythonAttrs == false then
        drv
      else
...
lib.composeManyExtensions [
  ...
  (
    ...
    in
    lib.mapAttrs (attr: systems: builtins.foldl' (drv: attr: addBuildSystem { inherit drv self attr; }) super.${attr} systems) buildSystems
  )
  ...
  (
...

    {
      ...
      ansible-base = super.ansible-base.overridePythonAttrs (
        ...
        // lib.optionalAttrs (lib.versionOlder old.version "2.4") { prePatch = ''sed -i "s,/usr/,$out," lib/ansible/constants.py''; }
      );
      ...
      argcomplete = super.argcomplete.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.importlib-metadata ]; });

      arpeggio = super.arpeggio.overridePythonAttrs (old: { nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ self.pytest-runner ]; });

      astroid = super.astroid.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      cairosvg = super.cairosvg.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      ccxt = super.ccxt.overridePythonAttrs (
        old: {
          preBuild = ''
            ln -s README.{rst,md}
          '';
      ...
      celery = super.celery.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.setuptools ]; });

      cerberus = super.cerberus.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.setuptools ]; });

      cssselect2 = super.cssselect2.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      configparser = super.configparser.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.toml ]; });

      confluent-kafka = super.confluent-kafka.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ pkgs.rdkafka ]; });

      copier = super.copier.overrideAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ pkgs.git ]; });
      ...
      cryptography =
        ...
        scrypto.overridePythonAttrs (
          ...
          // lib.optionalAttrs (lib.versionAtLeast old.version "3.4" && lib.versionOlder old.version "3.5") { CRYPTOGRAPHY_DONT_BUILD_RUST = "1"; }
      ...
      darts = super.darts.override { preferWheel = true; };

      datadog-lambda = super.datadog-lambda.overridePythonAttrs (
        old: {
          postPatch = ''
      ...
      dbus-python = super.dbus-python.overridePythonAttrs (
        old: {
          outputs = [
            "out"
            "dev"
      ...
      evdev = super.evdev.overridePythonAttrs (
        old: {
          preConfigure = ''
      ...
      fastparquet = super.fastparquet.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      flatbuffers = super.flatbuffers.overrideAttrs (old: { VERSION = old.version; });
      ...
      granian =
        ...
        super.granian.overridePythonAttrs (
          old: rec {
            ...
            cargoDeps = pkgs.rustPlatform.importCargoLock { lockFile = "${src.out}/Cargo.lock"; };
            nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [
      ...
      gitpython = super.gitpython.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.typing-extensions ]; });

      grpcio = super.grpcio.overridePythonAttrs (
        old: {
          nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ pkg-config ];
          buildInputs = (old.buildInputs or [ ]) ++ [
            pkgs.c-ares
      ...
      grpcio-tools = super.grpcio-tools.overridePythonAttrs (
        old: {
          outputs = [
            "out"
            "dev"
      ...
      gunicorn = super.gunicorn.overridePythonAttrs (
        old: {
      ...
      hikari-lightbulb = super.hikari-lightbulb.overrideAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.setuptools ]; });

      horovod = super.horovod.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ pkgs.mpi ]; });

      httplib2 = super.httplib2.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.pyparsing ]; });

      icecream = super.icecream.overridePythonAttrs (
        old: {
      ...
      ipython = super.ipython.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.setuptools ]; });

      isort = super.isort.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.setuptools ]; });

      jaraco-functools = super.jaraco-functools.overridePythonAttrs (
        old: {
          # required for the extra "toml" dependency in setuptools_scm[toml]
          buildInputs = (old.buildInputs or [ ]) ++ [ self.toml ];
        }
      );

      trio = super.trio.overridePythonAttrs (
        old: {
          propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [
      ...
      jeepney = super.jeepney.overridePythonAttrs (
        old: {
          buildInputs = (old.buildInputs or [ ]) ++ [
            self.outcome
            self.trio
      ...
      jq = super.jq.overridePythonAttrs (
        attrs: {
          buildInputs = [ pkgs.jq ];
          patches = [
            (pkgs.fetchpatch {
      ...
      jsonslicer = super.jsonslicer.overridePythonAttrs (
        old: {
      ...
      jsonschema =
        if lib.versionAtLeast super.jsonschema.version "4.0.0" then
          super.jsonschema.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.importlib-resources ]; })
        else
          super.jsonschema;
      ...
      lap = super.lap.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.numpy ]; });

      libarchive = super.libarchive.overridePythonAttrs (
        old: {
          buildInputs = (old.buildInputs or [ ]) ++ [ self.setuptools ];
      ...
      matplotlib = super.matplotlib.overridePythonAttrs (
        ...
        in
        {
          ...
          buildInputs =
            ...
            ++ lib.optionals (lib.versionAtLeast super.matplotlib.version "3.7.0") [ self.pybind11 ];
      ...
      multiaddr = super.multiaddr.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      omegaconf = super.omegaconf.overridePythonAttrs (old: { nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ pkgs.jdk ]; });
      ...
      parsel = super.parsel.overridePythonAttrs (old: { nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      poethepoet = super.poethepoet.overrideAttrs (old: { propagatedBuildInputs = old.propagatedBuildInputs ++ [ self.poetry ]; });

      poetry-core = super.poetry-core.overridePythonAttrs (
        old:
        let
          initFile = if lib.versionOlder super.poetry-core.version "1.1" then "poetry/__init__.py" else "./src/poetry/core/__init__.py";
        in
        {
          ...
          super.pyarrow.overridePythonAttrs (
            ...
            let
              ...
              _arrow-cpp = pkgs.arrow-cpp.override (
                builtins.intersectAttrs (lib.functionArgs pkgs.arrow-cpp.override) {
                  python = self.python;
                  python3 = self.python;
                }
              );
      ...
      pycairo =
        (
          drv:
          (drv.overridePythonAttrs (_: { format = "other"; })).overridePythonAttrs (
            old: {
      ...
      pycocotools = super.pycocotools.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.numpy ]; });

      pyfftw = super.pyfftw.overridePythonAttrs (
        old: {
          buildInputs = (old.buildInputs or [ ]) ++ [
            pkgs.fftw
            pkgs.fftwFloat
      ...
      pyfuse3 = super.pyfuse3.overridePythonAttrs (
        old: {
          nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ pkg-config ];
      ...
      pynput = super.pynput.overridePythonAttrs (
        old: {
          nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ self.sphinx ];

          propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.setuptools-lint ];
        }
      );

      pymssql = super.pymssql.overridePythonAttrs (
        old: {
          buildInputs = (old.buildInputs or [ ]) ++ [
            pkgs.openssl
            pkgs.libkrb5
          ];
          propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ pkgs.freetds ];
        }
      );
      ...
      pyrealsense2 = super.pyrealsense2.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ pkgs.libusb1.out ]; });
      ...
      python-snap7 = super.python-snap7.overridePythonAttrs (
        old: {
          propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ pkgs.snap7 ];
      ...
      pytoml = super.pytoml.overridePythonAttrs (old: { doCheck = false; });
      ...
      python-snappy = super.python-snappy.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ pkgs.snappy ]; });

      python-twitter = super.python-twitter.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });

      pythran = super.pythran.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });

      ffmpeg-python = super.ffmpeg-python.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });

      python-prctl = super.python-prctl.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ pkgs.libcap ]; });

      pyudev = super.pyudev.overridePythonAttrs (
        old: {
          postPatch = ''
      ...
      panel = super.panel.overridePythonAttrs (old: { nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ pkgs.nodejs ]; });
      ...
      rmfuse = super.rmfuse.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.setuptools ]; });
      ...
      shellcheck-py = super.shellcheck-py.overridePythonAttrs (
        old: {
      ...
      soundfile = super.soundfile.overridePythonAttrs (
        old: {
          postPatch = ''
      ...
      suds = super.suds.overridePythonAttrs (
        old: {
      ...
      systemd-python = super.systemd-python.overridePythonAttrs (
        old: {
      ...
      tinycss2 = super.tinycss2.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });
      ...
      typed_ast = super.typed-ast.overridePythonAttrs (old: { nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ self.pytest-runner ]; });

      urwidtrees = super.urwidtrees.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.urwid ]; });
      ...
      vispy = super.vispy.overrideAttrs (
        old: {
          ...
          nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ self.setuptools-scm-git-archive ];
        }
      );
      ...
      sentencepiece = super.sentencepiece.overridePythonAttrs (
        old: {
          ...
          buildInputs = (old.buildInputs or [ ]) ++ [ pkgs.sentencepiece ];
        }
      );
      ...
      cytoolz = super.cytoolz.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.toolz ]; });
      ...
      marisa-trie = super.marisa-trie.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pytest-runner ]; });

      ua-parser = super.ua-parser.overridePythonAttrs (old: { propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.pyyaml ]; });
      ...
      pysqlite = super.pysqlite.overridePythonAttrs (
        old: {
      ...
      selinux = super.selinux.overridePythonAttrs (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.setuptools-scm-git-archive ]; });

      setuptools-scm = super.setuptools-scm.overridePythonAttrs (
        old: {
      ...
      uwsgi = super.uwsgi.overridePythonAttrs (
        ...
        // lib.optionalAttrs (lib.versionAtLeast old.version "2.0.19" && lib.versionOlder old.version "2.0.20") { sourceRoot = "."; }
      );
      ...
      mkdocs = super.mkdocs.overridePythonAttrs (old: { propagatedBuildInputs = old.propagatedBuildInputs or [ ] ++ [ self.babel ]; });
