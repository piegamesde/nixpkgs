...
let
  ...
  makeRemoveSpecialDependenciesHook =
    ...
    nonOverlayedPython.pkgs.callPackage
      ...
      { };
  makeSetupHookArgs =
    deps: if lib.elem "propagatedBuildInputs" (builtins.attrNames (builtins.functionArgs makeSetupHook)) then { propagatedBuildInputs = deps; } else { inherit deps; };
in
{
  ...
  wheelUnpackHook = callPackage (_: makeSetupHook { name = "wheel-unpack-hook.sh"; } ./wheel-unpack-hook.sh) { };
}
