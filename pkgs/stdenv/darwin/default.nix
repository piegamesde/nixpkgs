...
[
  ...
  (
    ...
    assert (!useAppleSDKLibs)
      -> lib.all isFromBootstrapFiles (
        with prevStage.darwin; [
          CF
          Libsystem
        ]
      ...
      overrides = self: super: {
        ...
        python3 =
          (super.python3.override {
            ...
            (_: { pname = "python3-minimal-scproxy"; });
  ...
  (
    ...
    assert lib.getVersion prevStage.stdenv.cc.bintools.bintools
  ...
  (
    ...
    assert (!useAppleSDKLibs)
      -> lib.all isBuiltByNixpkgsCompiler (with prevStage.darwin; [ Libsystem ]);
    assert useAppleSDKLibs
      -> lib.all isFromNixpkgs (
        with prevStage.darwin; [
          CF
  ...
  (
    ...
    assert (!useAppleSDKLibs)
      -> lib.all isBuiltByBootstrapFilesCompiler (with prevStage.darwin; [ configd ]);
    assert (!useAppleSDKLibs)
