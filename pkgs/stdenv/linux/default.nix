...
#     $ nix-tree --derivation $(nix-instantiate -A stdenv)
{
  ...
  bootstrapFiles ? let
    ...
    files = archLookupTable.${localSystem.system} or (if getCompatibleTools != null then getCompatibleTools else (abort "unsupported platform for the pure Linux stdenv"));
  in
  files,
}:
