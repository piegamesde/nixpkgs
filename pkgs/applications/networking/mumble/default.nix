...
let
  generic = overrides: source:
    stdenv.mkDerivation (source // overrides // {
      ...
      patches = [
        ...
        (fetchpatch {
          url =
            "https://github.com/mumble-voip/mumble/commit/f8d47db318f302f5a7d343f15c9936c7030c49c4.patch";
  ...
  server = source:
    generic {
      ...
      configureFlags = [ "-D client=OFF" ]
        ++ lib.optional (!iceSupport) "-D ice=OFF" ++ lib.optionals iceSupport [
