      ...
      let
        ...
        overrides = lib.optionalAttrs (variant == "stable") {
          ...
          auto-complete-clang-async =
            super.auto-complete-clang-async.overrideAttrs (old: {
          ...
          hindent = (externalSrc super.hindent
            pkgs.haskellPackages.hindent).overrideAttrs
          ...
          tokei = super.tokei.overrideAttrs (attrs: {
            postPatch = attrs.postPatch or "" + ''
              substituteInPlace tokei.el \
                --replace 'tokei-program "tokei"' 'tokei-program "${
                  lib.getExe pkgs.tokei
                }"'
            '';
          });
          ...
          vterm = super.vterm.overrideAttrs (old: {
            ...
            buildInputs = old.buildInputs
              ++ [ self.emacs pkgs.libvterm-neovim ];
            cmakeFlags =
