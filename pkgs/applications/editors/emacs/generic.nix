...
let
  ...
  inherit (if variant == "macport" then llvmPackages_6.stdenv else stdenv) mkDerivation;
in
mkDerivation (
  finalAttrs:
  (
    ...
    // {
      ...
      meta = {
        ...
        platforms = if variant == "macport" then lib.platforms.darwin else lib.platforms.all;
