...
let
  ratify = (stdenv.mkDerivation {
    ...
    nativeBuildInputs =
      [ meson ninja pkg-config docbook2x docbook-xsl-nons wrapGAppsHook ];
    buildInputs = [ gtk3 gobject-introspection vala gtk-doc ];
  });
  ...
  chimara = (stdenv.mkDerivation {
    ...
    nativeBuildInputs =
      [ meson ninja pkg-config perl bison texinfo python3 glib wrapGAppsHook ];
    buildInputs = [
      gtk3
...
in stdenv.mkDerivation {
  ...
  nativeBuildInputs =
    [ meson ninja pkg-config inform7 python3 desktop-file-utils wrapGAppsHook ];
  buildInputs = [
    gettext
    libossp_uuid
    gtk3
