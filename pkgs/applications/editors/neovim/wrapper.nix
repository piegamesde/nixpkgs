{ stdenv, symlinkJoin, lib, makeWrapper, writeText, nodePackages, python3
, python3Packages, callPackage, neovimUtils, vimUtils }:
neovim:
...
let
  wrapper = { extraName ? ""
    ...
    , manifestRc ? null, withPython2 ? false, withPython3 ? true
    , python3Env ? python3, withNodeJs ? false, rubyEnv ? null, vimAlias ? false
    , viAlias ? false
