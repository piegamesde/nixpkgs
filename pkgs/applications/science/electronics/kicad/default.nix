...
let
  ...
  kicadVersion = if srcOverridep "kicadVersion" then srcs.kicadVersion else versionsImport.${baseName}.kicadVersion.version;
  ...
  installPhase =
    ...
    (concatStringsSep "\n" (
      flatten [
        ...
        (map (tool: "makeWrapper ${base}/${bin}/${tool} $out/bin/${tool} $makeWrapperArgs" + optionalString (withScripting) " --set PYTHONPATH \"$program_PYTHONPATH\"") tools)
