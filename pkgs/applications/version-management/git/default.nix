...
in
  stdenv.mkDerivation (finalAttrs: {
    pname = "git" + lib.optionalString svnSupport "-with-svn"
      + lib.optionalString (!svnSupport && !guiSupport && !sendEmailSupport
        && !withManual && !pythonSupport && !withpcre2) "-minimal";
    inherit version;
    ...
    NIX_LDFLAGS = lib.optionalString
      (stdenv.cc.isGNU && stdenv.hostPlatform.libc == "glibc") "-lgcc_s"
