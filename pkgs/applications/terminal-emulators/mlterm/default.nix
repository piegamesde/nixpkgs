...
, nixosTests, vte, harfbuzz # can be replaced with libotf
, fribidi, m17n_lib, libssh2 # build-in ssh
, fcitx5, fcitx5-gtk, ibus, uim # IME
, wrapGAppsHook # color picker in mlconfig
, gdk-pixbuf, gtk3, gtk ? gtk3
...
, enableGuis ? {
  ...
  sdl2 = true;
}, libxkbcommon, wayland # for the "wayland" --with-gui option
...
let
  ...
  withFeaturesList = featureName: attrset:
    ...
    in lib.withFeatureAs (commaSepList != "") featureName commaSepList;
