...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru.fetchxrd =
    callPackage ./fetchxrd.nix { xrootd = finalAttrs.finalPackage; };
  passthru.tests = lib.optionalAttrs stdenv.hostPlatform.isLinux {
    test-runner =
      callPackage ./test-runner.nix { xrootd = finalAttrs.finalPackage; };
  } // {
    test-xrdcp = finalAttrs.passthru.fetchxrd {
      ...
      url =
        "root://eospublic.cern.ch//eos/opendata/alice/2010/LHC10h/000138275/ESD/0000/AliESDs.root";
  ...
  buildInputs =
    ...
    ++ lib.optionals enableTestRunner [ cppunit ];
  ...
  cmakeFlags = lib.optionals enableTestRunner [ "-DENABLE_TESTS=TRUE" ];
