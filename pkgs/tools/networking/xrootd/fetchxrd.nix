{ lib, runCommandLocal, buildPlatform, xrootd }:

{ name ? "", pname ? "", version ? "", urls ? [ ], url ? if urls == [ ] then
  abort "Expect either non-empty `urls` or `url`"
else
  builtins.head urls, hash ? lib.fakeHash }:
...
# TODO: Remove the library path after #200830 get merged
  ''
    for u in $urls; do
      ${lib.optionalString buildPlatform.isDarwin "DY"}LD_LIBRARY_PATH=${
