{ lib, stdenv, fetchFromGitHub, buildNpmPackage, makeWrapper, coreutils
, ffmpeg-headless, imagemagick_light, procps, python3, xorg
...
, geckodriver, firefox }:
assert (!withFirefox && !withChromium)
  -> throw "Either `withFirefox` or `withChromium` must be enabled.";
buildNpmPackage rec {
  pname = "sitespeed-io";
  version = "27.3.1";
  ...
  in ''
    wrapProgram $out/bin/sitespeed-io \
      ...
      ${
        lib.optionalString (!withFirefox && withChromium)
        "--add-flags '-b chrome'"
      } \
      ${
        lib.optionalString (withFirefox && !withChromium)
  ...
  meta = with lib; {
    ...
    platforms =
      lib.unique (geckodriver.meta.platforms ++ chromedriver.meta.platforms);
  };
}
