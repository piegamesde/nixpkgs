{ lib, stdenv, dotnetCorePackages, dotnet-runtime_6, dotnet-runtime_7
, buildDotnetModule, fetchFromGitHub, autoPatchelfHook, fontconfig, xorg
, libglvnd, makeDesktopItem, copyDesktopItems }:
...
let
  ...
  sdk = if lib.versionAtLeast (lib.versions.majorMinor version) "1.7" then
    dotnetCorePackages.sdk_7_0
  else
    dotnetCorePackages.sdk_6_0;

  runtime = if lib.versionAtLeast (lib.versions.majorMinor version) "1.7" then
    dotnet-runtime_7
  else
    dotnet-runtime_6;
