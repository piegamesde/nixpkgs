...
let
  ...
  opensshUnsafe = openssh.overrideAttrs (oldAttrs: { patches = oldAttrs.patches ++ [ ./openssh-nixos-sandbox.patch ]; });
in
  ...
  postFixup =
    let
      wrapperArgs =
        ...
        + lib.optionalString (builtins.length runtimeDeps > 0) " --prefix PATH : ${lib.makeBinPath runtimeDeps}";
