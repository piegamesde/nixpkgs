...
let
    ...
    runCommand "texlive-${pname}.bin-${version}"
      {
        ...
        scriptsFolder = lib.optionalString (tex ? outPath) (
          tex.outPath + "/scripts/" + args.scriptsFolder or pname
        );
        # binaries info
  ...
  man =
    builtins.removeAttrs
      (runCommand "texlive-${pname}.man-${version}${extraVersion}"
        {
          inherit meta texdoc;
          passthru = passthru // {
            tlOutputName = "man";
  ...
  info =
    builtins.removeAttrs
      (runCommand "texlive-${pname}.info-${version}${extraVersion}"
        {
          inherit meta texdoc;
          passthru = passthru // {
            tlOutputName = "info";
