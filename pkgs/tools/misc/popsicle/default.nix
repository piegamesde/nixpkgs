{ lib, stdenv, fetchFromGitHub, cargo, rustPlatform, rustc, glib, pkg-config
, gdk-pixbuf, gtk3, wrapGAppsHook }:
...
stdenv.mkDerivation rec {
  ...
  nativeBuildInputs =
    [ glib pkg-config rustPlatform.cargoSetupHook cargo rustc wrapGAppsHook ];
