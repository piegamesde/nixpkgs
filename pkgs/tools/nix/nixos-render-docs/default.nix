...
in python.pkgs.buildPythonApplication rec {
  ...
  nativeBuildInputs = with python.pkgs; [ setuptools pytestCheckHook ];

  propagatedBuildInputs = with python.pkgs; [ markdown-it-py mdit-py-plugins ];
  ...
  passthru.tests.typing = runCommand "${pname}-mypy" {
    nativeBuildInputs = [
      (python3.withPackages
        (ps: with ps; [ mypy pytest markdown-it-py mdit-py-plugins ]))
    ];
  } ''
    mypy --strict ${src}
    touch $out
